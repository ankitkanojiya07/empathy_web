...
export function Dashboard() {
  ...
  const [filterRole, setFilterRole] = useState<"all" | "user" | "listener">(
    "all"
  );
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (!listenersRes.ok) {
        throw new Error(
          `Failed to fetch listeners: ${listenersRes.statusText}`
        );
      }
      ...
      setError(
        error instanceof Error
          ? error.message
          : "Failed to fetch data. Please check if the API server is running."
      );
    } finally {
      setLoading(false);
    }
  ...
  const filteredListeners = listeners.filter((listener) => {
    const matchesSearch =
      (listener.bio?.toLowerCase().includes(searchTerm.toLowerCase()) ??
        false) ||
      (listener.country?.toLowerCase().includes(searchTerm.toLowerCase()) ??
        false) ||
      (listener.city?.toLowerCase().includes(searchTerm.toLowerCase()) ??
        false) ||
      (listener.area_of_expertise?.some((exp) =>
  ...
  const totalEarnings = listeners.reduce(
    (sum, l) => sum + l.earnings_balance,
    0
  );
  const verifiedListeners = listeners.filter(
    (l) => l.kyc_status === "verified"
  ).length;
      ...
      case "pending":
        return (
          <Badge className="bg-yellow-500 text-white">
            <ClockCircle size={12} weight="Bold" className="mr-1" />
            Pending
          </Badge>
        );
      case "rejected":
        return (
          <Badge className="bg-red-500 text-white">
            <CloseCircle size={12} weight="Bold" className="mr-1" />
            Rejected
          </Badge>
        );
      default:
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div className="flex gap-2">
                <Button
                  ...
                  className={
                    activeTab === "users"
                      ? "bg-gradient-to-r from-[#DC67FF] to-[#89BFF4] text-white"
                      : ""
                  }
                >
              ...
              {loading ? (
                ...
                >
                  <Refresh
                    size={32}
                    weight="Outline"
                    className="animate-spin text-[#DC67FF]"
                  />
                </div>
              ) : (
                <div
                            ...
                            <td className="px-6 py-4">
                              <div className="text-xs space-y-1">
                                <div className="flex items-center gap-1">
                                  ...
                                  />
                                  <span>
                                    ${listener.audio_call_per_minute_rate}/min
                                  </span>
                                </div>
                                <div className="flex items-center gap-1">
                                  <VideoFrame
                                  ...
                                  />
                                  <span>
                                    ${listener.video_call_per_minute_rate}/min
                                  </span>
                                </div>
                                <div className="flex items-center gap-1">
                                  <ChatRound
                                  ...
                                  />
                                  <span>
                                    ${listener.cost_per_chat_message}/msg
                                  </span>
                                </div>
                              </div>
                            </td>
